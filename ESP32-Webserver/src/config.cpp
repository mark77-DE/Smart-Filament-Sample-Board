...
bool loadConfigV2()
{
    ...
    // LED-Reihenfolge
    if (strcmp(ledOrder, "GRB") == 0)
    {
        CONFIGV2.ledHardware.order = LED_ORDER_GRB;
    }
    else if (strcmp(ledOrder, "RGB") == 0)
    {
...
void applyConfigV2()
{
    ...
    neoPixelType pixelType = getNeoPixelType(
        CONFIGV2.ledHardware.type,
        CONFIGV2.ledHardware.order);
    ...
    if (CONFIGV2.system.debugMode)
    {
        ...
        Serial.println(CONFIGV2.ledHardware.type);
        Serial.print(F(" LED_COLOR_ORDER = "));
        Serial.println(CONFIGV2.ledHardware.order);
...
bool updateConfigFromJsonV2(JsonDocument &doc)
{
    ...
    // --- LED-Hardware ---
    if (cfg["ledHardware"].is<JsonObject>())
    {
        JsonObject ledHardware = cfg["ledHardware"];

        Serial.print(F("LED hardware configuration update requested:"));
        Serial.print(F("  JSON type  = "));
        Serial.print(ledHardware["type"] | "error");
        Serial.print(F("  JSON order = "));
        Serial.println(ledHardware["order"] | "error");

        // Werte sicher als String auslesen
        String ledType = ledHardware["type"].as<String>();
        String ledOrder = ledHardware["order"].as<String>();

        // DEBUG
        Serial.println(F("[DEBUG] /api/updateConfig LED-Hardware:"));

        if (ledType.length() > 0)
            Serial.printf("  JSON type  = '%s'\n", ledType.c_str());
        else
            Serial.println(F("  JSON type  = <NULL>"));

        if (ledOrder.length() > 0)
            Serial.printf("  JSON order = '%s'\n", ledOrder.c_str());
        else
            Serial.println(F("  JSON order = <NULL>"));

        Serial.printf(
            "  BEFORE: CONFIGV2 type=%d order=%d\n",
            (int)CONFIGV2.ledHardware.type,
            (int)CONFIGV2.ledHardware.order);
        ...
        if (ledType.length() > 0)
        {
            ...
            else if (ledType == "SK6812_RGB")
            {
                CONFIGV2.ledHardware.type = LED_TYPE_SK6812_RGB;
            }
            else if (ledType == "SK6812_RGBW")
            {
                CONFIGV2.ledHardware.type = LED_TYPE_SK6812_RGBW;
            }
            else
...
bool saveConfigV2()
{
    ...
    Serial.printf(
        "[Config] Saving LED hardware: type=%d, order=%d\n",
        (int)CONFIGV2.ledHardware.type,
        (int)CONFIGV2.ledHardware.order);

    JsonObject ledHardware = doc["ledHardware"].to<JsonObject>();
    switch (CONFIGV2.ledHardware.type)
    {
    case LED_TYPE_WS2812B:
        ledHardware["type"] = "WS2812B";
        break;
    case LED_TYPE_SK6812_RGB:
        ledHardware["type"] = "SK6812_RGB";
        break;
    case LED_TYPE_SK6812_RGBW:
        ledHardware["type"] = "SK6812_RGBW";
        break;
    default:
        ledHardware["type"] = "WS2812B";
        break;
    ...
    switch (CONFIGV2.ledHardware.order)
    {
    case LED_ORDER_GRB:
        ledHardware["order"] = "GRB";
        break;
    case LED_ORDER_RGB:
        ledHardware["order"] = "RGB";
        break;
    case LED_ORDER_GRBW:
        ledHardware["order"] = "GRBW";
        break;
    default:
        ledHardware["order"] = "GRB";
        break;
...
bool importConfigJsonV2(JsonObject src)
{
    ...
    // LED-Hardware
    // =========================
    if (src["ledHardware"].is<JsonObject>())
    {
        JsonObject ledHardware = src["ledHardware"];
        ...
        if (CONFIGV2.system.debugMode)
        {
            Serial.println(F("LED hardware configuration imported:"));
            Serial.print(F("  LED type: "));
            Serial.println(static_cast<int>(CONFIGV2.ledHardware.type));
            Serial.print(F("  LED order: "));
            Serial.println(static_cast<int>(CONFIGV2.ledHardware.order));
        }
    }
