...
#define DBG(...)                                 \
  do                                             \
  {                                              \
    Serial.printf("[NFCLED][t=%lu] ", millis()); \
...
static inline void renderAll(uint32_t neo)
{
  ...
  for (int i = 0; i < n; ++i)
    LEDCTRL_NFC::rawStrip()->setPixelColor(i, neo);
}
...
{
  if (!idlePulseEnabled || !LEDCTRL_NFC::rawStrip())
    return;
...
void LEDCTRL_NFC::update()
{
  ...
  {
    if (now < s_idleBlockUntil || now < s_netPauseUntil)
      return;

    // Stabiler Takt: nicht auf "now" snappen, sondern Intervalle nachholen
    if ((uint32_t)(now - s_lastPulseUpdate) >= PULSE_INTERVAL_MS)
    {
...
void LEDCTRL_NFC::netBusyHint(uint16_t ms)
{
  ...
  if (until > s_netPauseUntil)
    s_netPauseUntil = until;
}
