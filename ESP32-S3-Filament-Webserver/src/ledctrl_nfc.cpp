...
unsigned long NFC_LED_TIMEOUT     = 6000;   // ms

uint32_t NFC_LED_COLOR_SUCCESS = 0x00FF00;  // 0xRRGGBB
uint32_t NFC_LED_COLOR_ERROR   = 0xFF0000;
uint32_t NFC_LED_COLOR_PULSE   = 0x0033AA;
...
static bool          s_successBlinkOn   = false;
static unsigned long s_blinkStartTs     = 0;   // feste Startzeit
static uint16_t      s_blinkMs          = 150; // verwendetes Intervall
static constexpr uint16_t MIN_BLINK_MS  = 25;  // Untergrenze
static constexpr uint8_t  MAX_BLINK_COUNT = 10;// Obergrenze

// Hold/Presence
static bool          s_tagHeld          = false;
static unsigned long s_lastTagSeen      = 0;
static const uint16_t TAG_HELD_GRACE_MS = 200;

// Reassert (gegen halbe Frames / RMT-Glitches)
static bool          s_holdActive       = false;
static uint32_t      s_holdColorNeo     = 0;
static unsigned long s_lastHoldRefresh  = 0;
static const uint16_t HOLD_REFRESH_MS   = 25;

// Timeout erst ab Tag-Entfernung
static unsigned long s_releaseTs        = 0; // 0 = kein Timeout aktiv

// Idle Pulse
static bool          idlePulseEnabled   = true;
static float         minBrightness      = 0.35f;
...
// Präsenz-Tracking. Timeout startet erst bei Falling-Edge.
...
void LEDCTRL_NFC::confirmSuccess() {
  ...
  // Fallback: direkt stabiler Success
...
void LEDCTRL_NFC::confirmError() {
  ...
  s_lastHoldRefresh  = 0;
  s_idleBlockUntil   = now + 2; // nur Idle verzögern
  s_releaseTs        = s_tagHeld ? 0 : now;
...
void LEDCTRL_NFC::update() {
  ...
  // SUCCESS BLINK (phasenbasiert, 1× render/show pro Tick)
  ...
  // IDLE (Pulse) — Idle-Blocker wirkt NUR hier
